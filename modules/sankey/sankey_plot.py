...
def draw_sankey(name, path):
    ...
    """
    if len(name) == 1:
        df = pd.read_csv(f'{path}/lineage-{name[0]}.csv', sep = ',')
    ...
    df_labels['hover_label'] = df_labels[['LABEL_NODE','FILTER', 'ON']].apply(lambda x : '{}<br />Filter: {}<br />On: {}'.format(x[0],x[1], x[2]), axis=1)
